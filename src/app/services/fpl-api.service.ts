...
export interface FplManager { id: number; player_first_name: string; player_last_name: string; name: string; summary_overall_points: number; summary_overall_rank: number; summary_event_points: number; current_event: number; leagues?: { classic: FplLeague[]; h2h: unknown[]; cup: unknown; }; }
export interface FplLeague { id: number; name: string; created: string; admin_entry: number | null; rank: number | null; entry_rank: number | null; entry_last_rank: number | null; rank_count?: number; start_event?: number; }
export interface BootstrapStatic { events: Array<{ id: number; name: string; deadline_time: string; finished: boolean; is_current: boolean; is_next: boolean; }>; }
export interface FplStanding { entry: number; entry_name: string; event_total: number; last_rank: number; player_name: string; rank: number; rank_sort: number; total: number; club_badge_src: string | null; }
...
export class FplApiService {
  ...
  getBootstrap(): Observable<BootstrapStatic> { return this.http.get<BootstrapStatic>(`${this.apiBase}/bootstrap-static/`); }
